...
const snoise = Fn(([uvIn, res]: [any, any]) => {
	...
	// biome-ignore lint/suspicious/noExplicitAny: The inferred scalar type omits valid vector swizzles.
...
const fireColorNode = Fn(() => {
	...
	const coord = vec3(atan(p.x, p.y).div(6.2832).add(0.5), length(p).mul(0.4), 0.5);

	const drift = vec3(0.0, time.mul(-0.05), time.mul(0.01));
	for (let i = 1; i <= 7; i++) {
		const power = 2.0 ** i;
		color = color.add(float(1.5 / power).mul(snoise(coord.add(drift), float(power * 16.0))));
	}
