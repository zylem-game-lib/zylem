 ...
 */
import { Fn, abs, cos, exp, float, fract, length, pow, sin, time, uv, vec3, vec4 } from 'three/tsl';
import type { ZylemTSLShader } from '../types';

/** Cosine gradient palette (iq). */
// biome-ignore lint/suspicious/noExplicitAny: Three.js cannot infer TSL Fn node parameters reliably.
...
const starColorNode = Fn(() => {
	...
	// biome-ignore lint/suspicious/noExplicitAny: TSL arithmetic changes the inferred node shape.
	let p: any = uv0;
	// biome-ignore lint/suspicious/noExplicitAny: TSL arithmetic changes the inferred node shape.
	...
	for (let i = 0; i < 4; i++) {
		...
		// biome-ignore lint/suspicious/noExplicitAny: Reassigned TSL expressions do not share a stable node type.
