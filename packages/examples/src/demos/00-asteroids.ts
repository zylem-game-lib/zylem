...
const asteroidImages = [meteorBig1, meteorBig2, meteorBig3, meteorBig4] as const;

function getRotationAngle2D(
...
playerShip.onUpdate(({ me, inputs, delta }) => {
	...
	if (brakeInput > 0 && me.body) {
		...
		const brakeScale = Math.max(0, 1 - brakeInput * BRAKE_DECELERATION * delta);
		me.body.setLinvel({
...
const asteroids = Array.from({ length: 5 }, (_, index) => createAsteroid(index));
...
const game = createGame({
	...
	.onGlobalChange<number>('score', (score, currentStage) => {
		currentStage?.getEntityByName('scoreText', TEXT_TYPE)?.updateText(`Score: ${score}`);
	});
