...
const colorB = vec3(0.03, 0.22, 0.8);
const colorC = vec3(0.05, 0.3, 0.95);
const speed = 0.3;
...
const planetColorNode = Fn(() => {
  ...
  // Animate cheaply
  const animOffset = vec3(cos(t.mul(0.2)), sin(t.mul(0.2)), sin(t.mul(0.2)));
  ...
  const col2 = mix(
    col1,
    colorC,
    clamp(q.y.mul(0.5).add(0.5), float(0.0), float(1.0)),
  );
