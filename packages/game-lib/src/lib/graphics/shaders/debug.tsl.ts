 ...
 * space derivative (`fwidth`) of the surface `uv()` quad edges. It is exported
 * for API completeness but is not consumed by any demo; wire up a barycentric
 * geometry attribute if a true triangle wireframe is needed.
 */
import {
	Fn,
	uv,
...
const debugColorNode = Fn(() => {
	...
	const dist = min(
		min(uvc.x, float(1.0).sub(uvc.x)),
		min(uvc.y, float(1.0).sub(uvc.y)),
	);
