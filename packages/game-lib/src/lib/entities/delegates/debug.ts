...
export class DebugDelegate {
  ...
  private getPhysicsInfo(): Record<string, any> | null {
    ...
    info.velocity = `${velocity.x.toFixed(2)}, ${velocity.y.toFixed(
      2,
...
class EnhancedDebugInfoBuilder {
  ...
  constructor(
    customBuilder?: (options: GameEntityOptions) => Record<string, any>,
  ) {
    this.customBuilder = customBuilder;
  }

  buildInfo(
    options: GameEntityOptions,
    ...
    const { x, y, z } = normalizeVec3(options.position, VEC3_ZERO);
    return {
