...
type ZylemCooldownIconOptions = GameEntityOptions & {
   ...
   * Accepts a `Vector2`, plain `{ x, y }` object, or tuple.
   * @default { x: 0, y: 0 }
   */
...
export class ZylemCooldownIcon extends GameEntity<ZylemCooldownIconOptions> {
  ...
  constructor(options?: ZylemCooldownIconOptions) {
    super();
    this.options = deepMergeValues(
      cooldownIconDefaults,
      options,
    ) as ZylemCooldownIconOptions;
    this.options.screenPosition = toThreeVector2(
      this.options.screenPosition,
      cooldownIconDefaults.screenPosition,
    );
  ...
  private createSprite(): void {
    ...
    if (this.options.icon) {
      ...
      img.onload = () => {
        this._iconImage = img;
        this.redraw(
          this._lastRenderedProgress >= 0 ? this._lastRenderedProgress : 1,
        );
      };
  ...
  private iconUpdate(_params: UpdateContext<ZylemCooldownIconOptions>): void {
    ...
    if (
      Math.abs(progress - this._lastRenderedProgress) > 0.02 ||
      (progress === 1 && this._lastRenderedProgress !== 1)
    ) {
  ...
  private resolveScreenPosition(
    ...
    const pos = normalizeVec2(this.options.screenPosition, VEC2_ZERO);
    const ox = pos.x;
    const oy = pos.y;
  ...
  private updateStickyTransform(): void {
    ...
    const camera = this._cameraRef.camera as
      | PerspectiveCamera
      | OrthographicCamera;
 ...
 * ```
 */
export function createCooldownIcon(
  ...args: Array<CooldownIconOptions>
): ZylemCooldownIcon {
  const configArgs = args.filter(
    (arg): arg is Partial<ZylemCooldownIconOptions> =>
      !(arg instanceof BaseNode),
  );
